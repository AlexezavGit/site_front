...
export default function Home() {
  return (
    <div>
      ...
      <section className="py-16"><div className="container"><div className="grid md:grid-cols-3 gap-6"><div className="rounded-xl p-6" style={{ background: "rgba(37,99,235,0.06)", border: "1px solid rgba(37,99,235,0.2)" }}><AlertCircle style={{ color: "#2563EB", width: 32, height: 32, marginBottom: 12 }} /><h3 className="font-bold text-lg mb-2" style={{ color: NAVY }}>Потрібна допомога?</h3><p className="text-sm text-muted-foreground mb-4">Ступенева програма підтримки — від першої консультації до клінічної реабілітації.</p><Link href="/beneficiaries"><Button className="w-full">Бенефіціарам →</Button></Link></div><div className="rounded-xl p-6" style={{ background: "rgba(22,163,74,0.06)", border: "1px solid rgba(22,163,74,0.2)" }}><Users style={{ color: "#16A34A", width: 32, height: 32, marginBottom: 12 }} /><h3 className="font-bold text-lg mb-2" style={{ color: NAVY }}>Ви фахівець?</h3><p className="text-sm text-muted-foreground mb-4">Вийдіть із тіні. Отримайте сертифікацію, доступ до пацієнтів і стале фінансування.</p><Link href="/pro"><Button variant="outline" className="w-full border-green-400 text-green-700 hover:bg-green-100">Фахівцям →</Button></Link></div><div className="rounded-xl p-6" style={{ background: "rgba(212,160,23,0.06)", border: "1px solid rgba(212,160,23,0.25)" }}><Shield style={{ color: GOLD, width: 32, height: 32, marginBottom: 12 }} /><h3 className="font-bold text-lg mb-2" style={{ color: NAVY }}>Хочете підтримати?</h3><p className="text-sm text-muted-foreground mb-4">Де-ризиковане співфінансування. Прозора підзвітність. Вимірюваний вплив.</p><Link href="/donors"><Button variant="outline" className="w-full border-amber-400 text-amber-700 hover:bg-amber-100">Донорам →</Button></Link></div></div><div className="mt-10 text-center"><a href="https://dashboard-1q7.pages.dev/" target="_blank" rel="noopener noreferrer" style={{ display: "inline-flex", alignItems: "center", gap: "0.4rem", color: "rgba(0,0,0,0.45)", fontSize: "0.8125rem", textDecoration: "none" }}><ExternalLink style={{ width: 13, height: 13 }} />Відкрити аналітичний дашборд</a></div></div></section>
    </div>
  );
}
